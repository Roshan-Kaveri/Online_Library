...
const RazorpayButton = ({ amount, user, bookId }) => {
  ...
  const pay = async () => {
    ...
    const result = await fetch("http://online-library-backend-six.vercel.app/rzpay/create", {
      method: "POST",
    ...
    const options = {
      ...
      handler: async function (response) {
        const verifyRes = await fetch("http://online-library-backend-six.vercel.app/rzpay/success", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ...response,
